...
public class DatumCode implements ICodeData {
    ...
    private static char toU8(int value) {
    ...
    public DatumCode(byte[] apSsid, byte[] apBssid, byte[] apPassword, InetAddress ipAddress, ITouchEncryptor encryptor) {
        char length = toU8(apPassword.length);
        CRC8 crc8 = new CRC8();
        crc8.update(apSsid);
        char value = toU8((int) crc8.getValue());
        crc8.reset();
        crc8.update(apBssid);
        char value2 = toU8((int) crc8.getValue());
        char length2 = toU8(apSsid.length);
        byte[] address = ipAddress.getAddress();
        int length3 = address.length;
        char c = toU8(length3 + 5 + length + length2);
        ...
        for (int i2 = 0; i2 < apPassword.length; i2++) {
            ...
            c2 = toU8(c2 ^ cConvertByte2Uint82);
        ...
        for (int i3 = 0; i3 < apSsid.length; i3++) {
            ...
            c2 = toU8(c2 ^ cConvertByte2Uint83);
