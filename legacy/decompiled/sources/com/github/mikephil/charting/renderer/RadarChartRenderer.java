...
public class RadarChartRenderer extends LineRadarRenderer {
    ...
    protected void drawDataSet(Canvas canvas, IRadarDataSet iRadarDataSet, int i) {
        ...
        for (int i2 = 0; i2 < iRadarDataSet.getEntryCount(); i2++) {
            ...
            if (!Float.isNaN(mPPointF.f76x)) {
                if (!z) {
                    path.moveTo(mPPointF.f76x, mPPointF.f77y);
                    z = true;
                } else {
                    path.lineTo(mPPointF.f76x, mPPointF.f77y);
                }
            }
        }
        if (iRadarDataSet.getEntryCount() > i) {
            path.lineTo(centerOffsets.f76x, centerOffsets.f77y);
        }
        path.close();
    ...
    public void drawValues(Canvas canvas) {
        ...
        while (i4 < ((RadarData) this.mChart.getData()).getDataSetCount()) {
            ...
            if (shouldDrawValues(dataSetByIndex)) {
                ...
                mPPointF6.f76x = Utils.convertDpToPixel(mPPointF6.f76x);
                mPPointF6.f77y = Utils.convertDpToPixel(mPPointF6.f77y);
                int i5 = 0;
                while (i5 < dataSetByIndex.getEntryCount()) {
                    ...
                    if (dataSetByIndex.isDrawValuesEnabled()) {
                        ...
                        f4 = sliceAngle;
                        mPPointF3 = mPPointF5;
                        drawValue(canvas, dataSetByIndex.getValueFormatter(), radarEntry.getY(), radarEntry, i4, mPPointF4.f76x, mPPointF4.f77y - fConvertDpToPixel, dataSetByIndex.getValueTextColor(i5));
                    } else {
                        i2 = i5;
                        iRadarDataSet = dataSetByIndex;
                        i3 = i4;
                    ...
                    if (radarEntry.getIcon() != null && iRadarDataSet.isDrawIconsEnabled()) {
                        ...
                        Utils.getPosition(centerOffsets, (radarEntry.getY() * factor * phaseY) + mPPointF2.f77y, f5 + this.mChart.getRotationAngle(), mPPointF3);
                        mPPointF3.f77y += mPPointF2.f76x;
                        Utils.drawImage(canvas, icon, (int) mPPointF3.f76x, (int) mPPointF3.f77y, icon.getIntrinsicWidth(), icon.getIntrinsicHeight());
                    }
                    i5 = i2 + 1;
                    mPPointF6 = mPPointF2;
                    mPPointF5 = mPPointF3;
    ...
    protected void drawWeb(Canvas canvas) {
        ...
        for (int i = 0; i < entryCount; i += skipWebLineCount) {
            ...
            canvas.drawLine(centerOffsets.f76x, centerOffsets.f77y, mPPointF.f76x, mPPointF.f77y, this.mWebPaint);
        }
        MPPointF.recycleInstance(mPPointF);
        ...
        for (int i3 = 0; i3 < i2; i3++) {
            ...
            while (i4 < ((RadarData) this.mChart.getData()).getEntryCount()) {
                ...
                canvas.drawLine(mPPointF2.f76x, mPPointF2.f77y, mPPointF3.f76x, mPPointF3.f77y, this.mWebPaint);
            }
        }
    ...
    */
    ...
    public void drawHighlightCircle(Canvas canvas, MPPointF mPPointF, float f, float f2, int i, int i2, float f3) {
        ...
        if (i != 1122867) {
            ...
            path.reset();
            path.addCircle(mPPointF.f76x, mPPointF.f77y, fConvertDpToPixel, Path.Direction.CW);
            if (fConvertDpToPixel2 > 0.0f) {
                path.addCircle(mPPointF.f76x, mPPointF.f77y, fConvertDpToPixel2, Path.Direction.CCW);
            }
        ...
        if (i2 != 1122867) {
            ...
            canvas.drawCircle(mPPointF.f76x, mPPointF.f77y, fConvertDpToPixel, this.mHighlightCirclePaint);
        }
        canvas.restore();
    }
}
