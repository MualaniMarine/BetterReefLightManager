...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() { // from class: androidx.recyclerview.widget.DiffUtil.1
        ...
        public int compare(Snake snake, Snake snake2) {
            int i = snake.f36x - snake2.f36x;
            return i == 0 ? snake.f37y - snake2.f37y : i;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback callback, boolean z) {
        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (snakeDiffPartial != null) {
                ...
                snakeDiffPartial.f36x += range.oldListStart;
                snakeDiffPartial.f37y += range.newListStart;
                Range range2 = arrayList3.isEmpty() ? new Range() : (Range) arrayList3.remove(arrayList3.size() - 1);
                range2.oldListStart = range.oldListStart;
                range2.newListStart = range.newListStart;
                if (snakeDiffPartial.reverse) {
                    range2.oldListEnd = snakeDiffPartial.f36x;
                    range2.newListEnd = snakeDiffPartial.f37y;
                } else if (snakeDiffPartial.removal) {
                    range2.oldListEnd = snakeDiffPartial.f36x - 1;
                    range2.newListEnd = snakeDiffPartial.f37y;
                } else {
                    range2.oldListEnd = snakeDiffPartial.f36x;
                    range2.newListEnd = snakeDiffPartial.f37y - 1;
                }
                arrayList2.add(range2);
                if (snakeDiffPartial.reverse) {
                    if (snakeDiffPartial.removal) {
                        range.oldListStart = snakeDiffPartial.f36x + snakeDiffPartial.size + 1;
                        range.newListStart = snakeDiffPartial.f37y + snakeDiffPartial.size;
                    } else {
                        range.oldListStart = snakeDiffPartial.f36x + snakeDiffPartial.size;
                        range.newListStart = snakeDiffPartial.f37y + snakeDiffPartial.size + 1;
                    }
                } else {
                    range.oldListStart = snakeDiffPartial.f36x + snakeDiffPartial.size;
                    range.newListStart = snakeDiffPartial.f37y + snakeDiffPartial.size;
    ...
    */
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (snake != null && snake.f36x == 0 && snake.f37y == 0) {
                return;
            }
        ...
        private void findMatchingItems() {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                int i3 = snake.f36x + snake.size;
                int i4 = snake.f37y + snake.size;
                ...
                for (int i5 = 0; i5 < snake.size; i5++) {
                    int i6 = snake.f36x + i5;
                    int i7 = snake.f37y + i5;
        ...
        private boolean findMatchingItem(int i, int i2, int i3, boolean z) {
            ...
            while (i3 >= 0) {
                ...
                int i7 = snake.f36x + snake.size;
                int i8 = snake.f37y + snake.size;
                if (z) {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback listUpdateCallback) {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                Snake snake = this.mSnakes.get(size);
                int i3 = snake.size;
                int i4 = snake.f36x + i3;
                int i5 = snake.f37y + i3;
                if (i4 < i) {
                ...
                for (int i6 = i3 - 1; i6 >= 0; i6--) {
                    if ((this.mOldItemStatuses[snake.f36x + i6] & 31) == 2) {
                        batchingListUpdateCallback.onChanged(snake.f36x + i6, 1, this.mCallback.getChangePayload(snake.f36x + i6, snake.f37y + i6));
                    }
                }
                i = snake.f36x;
                i2 = snake.f37y;
